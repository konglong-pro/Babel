...
Matter is Babel's local, single-user physics notebook for knowledge, selected
...
```powershell
npm.cmd run dev:matter
npm.cmd run check -w @babel-apps/matter
```

Open `http://127.0.0.1:3008`. Private data lives in `data/matter/sqlite.db` and
the `data/matter/uploads/notes/` directory in the independent private `data/`
repository. The launcher supplies absolute `MATTER_DATABASE_PATH` and
`MATTER_NOTE_UPLOAD_DIRECTORY` values.
...
logically, and 160 MiB on the multipart wire. Matter serializes managed-note
image mutations and reconciles interrupted image transactions during health
checks and before later writes.

## Keyboard navigation

Matter follows Babel's Ready/Edit keyboard model. `Ctrl+F6` and
