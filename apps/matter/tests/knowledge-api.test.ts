...
before(async () => {
  temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), "matter-api-test-"));
  process.env.MATTER_DATABASE_PATH = path.join(temporaryDirectory, "sqlite.db");
  process.env.MATTER_NOTE_UPLOAD_DIRECTORY = path.join(temporaryDirectory, "uploads");
...
after(() => {
  ...
  assert.ok(resolved.startsWith(`${temporaryRoot}${path.sep}matter-api-test-`));
...
test("storage recovery restores owned quarantine and removes generated orphans", async () => {
  ...
  const staged = await noteImageStorage.stageNoteImages(
    "![Recovered](matter-upload://recovered)",
  ...
  const arbitraryPath = path.join(process.env.MATTER_NOTE_UPLOAD_DIRECTORY!, "personal.png");
      ...
      body: noteForm(
        {
          ...
          contentMd: "![Tiny](matter-upload://tiny)",
        },
        [["tiny", png, "image/png"]],
      ),
    }),
      ...
      body: noteForm(
        {
          ...
          contentMd: "![Rollback](matter-upload://rollback)",
        },
        ...
        body: noteForm(
          {
            ...
            contentMd:
              "[[Atomic image target]] ![Atomic](matter-upload://atomic)",
          },
      ...
      body: noteForm(
        {
          ...
          contentMd: "![Delete](matter-upload://delete)",
        },
...
async function uploadFiles(): Promise<string[]> {
  try {
    return (await readdir(process.env.MATTER_NOTE_UPLOAD_DIRECTORY!)).sort();
  } catch (error) {
    if (
