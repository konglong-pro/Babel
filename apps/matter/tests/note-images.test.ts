...
before(async () => {
  temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "matter-note-images-"));
  uploadDirectory = path.join(temporaryRoot, "uploads", "notes");
  previousUploadDirectory = process.env.MATTER_NOTE_UPLOAD_DIRECTORY;
  process.env.MATTER_NOTE_UPLOAD_DIRECTORY = uploadDirectory;
});

after(async () => {
  if (previousUploadDirectory === undefined) {
    delete process.env.MATTER_NOTE_UPLOAD_DIRECTORY;
  } else {
    process.env.MATTER_NOTE_UPLOAD_DIRECTORY = previousUploadDirectory;
  }
...
test("Matter note image storage and GET route", async (t) => {
  ...
  await t.test("stores every supported signature and serves safe immutable responses", async () => {
    ...
    for (const fixture of fixtures) {
      ...
      assert.match(
        imagePath,
        new RegExp(`^data/matter/uploads/notes/[A-Za-z0-9-]+\\.${fixture.extension}$`),
      );
  ...
  await t.test("rejects unsafe paths, invalid content, and oversized uploads", async () => {
    ...
    for (const candidate of [
      ...
      "data/matter/uploads/notes/../escape.png",
      "image.png\0",
    ]) {
      assert.throws(
  ...
  await t.test("stages one-to-one tokens and recognizes only managed Markdown URLs", async () => {
    const staged = await stageNoteImages(
      [
        "![First](matter-upload://first)",
        "![Second](matter-upload://second)",
      ].join("\n"),
    ...
    assert.equal(staged.contentMd.includes("matter-upload://"), false);
    ...
    await assert.rejects(
      stageNoteImages("![Missing](matter-upload://missing)", new Map()),
      hasStorageCode("INVALID_CONTENT"),
    );
    await assert.rejects(
      stageNoteImages(
  ...
  await t.test("stages one upload pool across multiple Markdown sources", async () => {
    const staged = await stageNoteImagesInMarkdown(
      [
        "![Shared](matter-upload://shared)",
        "![Shared again](matter-upload://shared) ![Worked](matter-upload://worked)",
      ],
      new Map<string, NoteImageUpload>([
    ...
    assert.equal(staged.markdownSources.some((source) => source.includes("matter-upload://")), false);
    ...
    await assert.rejects(
      stageNoteImagesInMarkdown(
        [
          "![Answer](matter-upload://answer)",
          "![Missing](matter-upload://missing)",
        ],
  ...
  await t.test("rolls back partial staging and supports quarantine restore and finalize", async () => {
    ...
    await assert.rejects(
      stageNoteImages(
        "![Good](matter-upload://good) ![Bad](matter-upload://bad)",
        new Map<string, NoteImageUpload>([
