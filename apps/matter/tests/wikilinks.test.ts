...
before(async () => {
  temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), "matter-links-test-"));
...
after(() => {
  ...
  assert.ok(resolved.startsWith(`${path.resolve(os.tmpdir())}${path.sep}matter-links-test-`));
...
test("link trigger failures roll back entity and relation writes", () => {
  ...
  database.sqlite.exec(`
    CREATE TRIGGER fail_matter_link_insert
  ...
  try {
    ...
    database.sqlite.exec("DROP TRIGGER fail_matter_link_insert");
  }
});
