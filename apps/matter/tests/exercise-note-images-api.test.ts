...
before(async () => {
  temporaryDirectory = await mkdtemp(
    path.join(os.tmpdir(), "matter-exercise-markdown-api-"),
  );
  previousDatabasePath = process.env.MATTER_DATABASE_PATH;
  previousNoteUploadDirectory = process.env.MATTER_NOTE_UPLOAD_DIRECTORY;
...
after(() => {
  ...
  restoreEnvironment("MATTER_NOTE_UPLOAD_DIRECTORY", previousNoteUploadDirectory);

  const resolved = path.resolve(temporaryDirectory);
  const temporaryRoot = path.resolve(os.tmpdir());
  assert.ok(
    resolved.startsWith(`${temporaryRoot}${path.sep}matter-exercise-markdown-api-`),
  );
...
test("exercise Markdown images share ownership across problem, answer, and solution", async () => {
      ...
      body: noteForm(
        {
          ...
          problemMd: [
            "Problem text",
            "![Shared problem](matter-upload://shared)",
            "![Problem only](matter-upload://problem)",
          ].join("\n"),
          answerMd: [
            "![Shared answer](matter-upload://shared)",
            "![Answer only](matter-upload://answer)",
          ].join("\n"),
          solutionMd: [
            "![Shared solution](matter-upload://shared)",
            "![Solution only](matter-upload://solution)",
          ].join("\n"),
        },
        [
  ...
  assert.equal(created.problemMd.includes("matter-upload://"), false);
  assert.equal(created.answerMd.includes("matter-upload://"), false);
  assert.equal(created.solutionMd.includes("matter-upload://"), false);
  ...
  const patchResponse = await itemRoute.PATCH(
    new Request(`http://localhost/api/exercises/${created.id}`, {
      ...
      body: noteForm(
        {
          problemMd: [
            "![Valid](matter-upload://valid)",
            "![Invalid](matter-upload://invalid)",
          ].join("\n"),
        },
        [
          ["valid", png, "image/png"],
...
async function noteUploadFiles(): Promise<string[]> {
  try {
    return (await readdir(process.env.MATTER_NOTE_UPLOAD_DIRECTORY!))
...
async function noteStagingFiles(): Promise<string[]> {
  try {
    return (
      await readdir(path.join(process.env.MATTER_NOTE_UPLOAD_DIRECTORY!, ".staging"))
    ).sort();
  } catch (error) {
    if (isNodeError(error) && error.code === "ENOENT") return [];
    throw error;
