...
test("Matter database readiness", async (t) => {
  ...
  await t.test("health reports current and incompatible schemas", async () => {
    const originalPath = process.env.MATTER_DATABASE_PATH;
    const originalUploads = process.env.MATTER_NOTE_UPLOAD_DIRECTORY;
    process.env.MATTER_DATABASE_PATH = path.join(root, "current.db");
    process.env.MATTER_NOTE_UPLOAD_DIRECTORY = path.join(root, "uploads", "notes");
    await mkdir(process.env.MATTER_NOTE_UPLOAD_DIRECTORY, { recursive: true });
    assert.equal((await getHealth()).status, 200);
    const { sqlite } = await import("@/lib/db/client");
    sqlite.close();

    process.env.MATTER_DATABASE_PATH = path.join(root, "missing-parent.db");
    const originalConsoleError = console.error;
    console.error = () => undefined;
    try {
      assert.equal((await getHealth()).status, 503);
      const missingPath = path.join(root, "missing.db");
      process.env.MATTER_DATABASE_PATH = missingPath;
      assert.equal((await getHealth()).status, 503);
      assert.equal(existsSync(missingPath), false);
    } finally {
      console.error = originalConsoleError;
      if (originalPath === undefined) delete process.env.MATTER_DATABASE_PATH;
      else process.env.MATTER_DATABASE_PATH = originalPath;
      if (originalUploads === undefined) delete process.env.MATTER_NOTE_UPLOAD_DIRECTORY;
      else process.env.MATTER_NOTE_UPLOAD_DIRECTORY = originalUploads;
    }
  });
});
