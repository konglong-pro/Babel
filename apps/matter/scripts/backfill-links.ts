...
try {
  ...
  console.log(
    `Matter link index rebuilt: ${result.sources} sources, ${result.links} links, ` +
  ...
  if (result.unresolved.length > 0) {
    ...
    for (const link of result.unresolved) {
      console.log(
        `- ${link.sourceKind} source ${link.sourceId} ` +
          `(${JSON.stringify(link.sourceTitle)}): ${JSON.stringify(link.targetTitleKey)}`,
      );
    }
  }
} finally {
