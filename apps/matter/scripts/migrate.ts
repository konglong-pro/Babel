...
async function main(): Promise<void> {
  try {
    ...
    if (violations.length > 0) {
      throw new Error(
        `Matter migration created ${violations.length} foreign-key violation(s). Restore the backup before retrying.`,
      );
    }
    ...
    console.log(
      `Matter database is up to date. Prepared ${prepared} and finalized ${finalized} legacy exercise problem image(s).`,
    );
  } finally {
    if (sqlite.open) {
      sqlite.pragma("foreign_keys = ON");
