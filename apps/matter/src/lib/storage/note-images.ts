...
export const NOTE_IMAGE_DIRECTORY = "data/matter/uploads/notes";
export const NOTE_UPLOAD_PLACEHOLDER_PREFIX = "matter-upload://";
...
export interface StagedNoteImageMarkdown {
...
export async function stageNoteImages(
  ...
  const staged = await stageNoteImagesInMarkdown([contentMd], uploads);
  return {
    contentMd: staged.markdownSources[0] ?? contentMd,
    imagePaths: staged.imagePaths,
  };
}

export async function stageNoteImagesInMarkdown(
  markdownSources: readonly string[],
  uploads: ReadonlyMap<string, NoteImageUpload>,
): Promise<StagedNoteImageMarkdown> {
  assertMarkdownSaveLimits(markdownSources, uploads);
  const tokens = new Set<string>();
  for (const source of markdownSources) {
    for (const token of uploadTokensInMarkdown(source)) tokens.add(token);
  }
  if (tokens.size !== uploads.size || [...tokens].some((token) => !uploads.has(token))) {
    throw new ImageStorageError(
      "INVALID_CONTENT",
      "Every image upload must have one matching placeholder in the Markdown content.",
    );
  }
  ...
  const updatedSources = markdownSources.map((source) => {
  ...
  try {
    assertMarkdownSaveLimits(updatedSources, uploads);
  } catch (error) {
    await discardStagedNoteImages(imagePaths, error);
  }

  return { markdownSources: updatedSources, imagePaths };
}
...
export function assertNoteSaveLimits(
  ...
  assertMarkdownSaveLimits([contentMd], uploads);
}

export function assertMarkdownSaveLimits(
  markdownSources: readonly string[],
  uploads: ReadonlyMap<string, NoteImageUpload>,
): void {
  let logicalBytes = 0;
  for (const source of markdownSources) {
    logicalBytes += utf8ByteLength(source);
    if (logicalBytes > NOTE_CONTENT_MAX_BYTES) {
      throw new ImageStorageError(
        "CONTENT_TOO_LARGE",
...
function uploadTokensInMarkdown(contentMd: string): Set<string> {
  const tokens = new Set<string>();
  const pattern = /matter-upload:\/\/([^\s)\]}'"<>]+)/g;
...
function resolveNoteImageRoot(): string {
  const configuredUploadDirectory = process.env.MATTER_NOTE_UPLOAD_DIRECTORY;
...
const markdownBaseUrl = new URL("https://matter.invalid/notes");
