...
const KNOWLEDGE_HEADING_ID_PREFIX = "matter-knowledge-heading-";
const REMARK_FEATURES = ["gfm", "formula-math"] as const;
const EMPTY_IMAGE_PREVIEWS: ReadonlyMap<string, string> = new Map();
const PENDING_IMAGE_URL_PATTERN = /matter-upload:\/\/[A-Za-z0-9._-]+/g;
...
export function KnowledgeDetail({
  ...
  const readerTriggerId = `matter-knowledge-${detail?.id ?? draftKey}-reader-trigger`;
      ...
      <header className="document-header">
        <div>
          <DetachedReaderWindow
            ...
            windowKey={`matter-knowledge-${detail.id}`}
            buttonLabel="Read"
          ...
          <button
            ...
            onClick={(event) => {
              prepareDetachedEditorWindow({
                ...
                windowKey: `matter-knowledge-content-${detail.id}`,
...
function KnowledgeForm({
  ...
  const activeImportedReferences = useMemo(
    () => (importDraft?.imageReferences ?? []).filter((reference) =>
      content.includes(`matter-upload://${reference.token}`),
    ),
  ...
  const referencedStagedImages = stagedImages.filter((image) =>
    content.includes(`matter-upload://${image.token}`),
  );
  ...
  function changeContent(nextContent: string) {
    ...
    setStagedImages((current) => {
      const retained = current.filter((image) => {
        const referenced = nextContent.includes(`matter-upload://${image.token}`);
        ...
        <header className="document-header">
          <div>
            <DetachedReaderWindow
              ...
              windowKey={`matter-knowledge-${detail?.id ?? draftKey}`}
              buttonLabel="Read"
              buttonPortalTargetId={readerTriggerId}
              disabled={pending}
            >
