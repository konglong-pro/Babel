...
export function MatterPageTabs() {
  return <NextPageTabs />;
}

export function MatterWorkspaceProcessHost({ children }: { children: ReactNode }) {
  return (
    <Suspense fallback={children}>
      <MatterWorkspaceProcessRuntime>{children}</MatterWorkspaceProcessRuntime>
    </Suspense>
  );
}

function MatterWorkspaceProcessRuntime({ children }: { children: ReactNode }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const activeProcess = matterWorkspaceProcess(pathname);
  ...
  const processes: WorkspaceProcessDefinition[] = MATTER_ARCHIVE_PROCESSES.map((type) => {
    const target = type === activeProcess ? archiveTarget : EMPTY_ARCHIVE_TARGET;
    return {
      ...matterWorkspaceRegistration(type),
      content: (
        <ArchiveWorkspace
          type={type}
  ...
  if (exerciseId !== null && activeProcess !== null) {
    processes.push({
      ...matterWorkspaceRegistration(activeProcess),
