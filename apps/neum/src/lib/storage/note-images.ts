...
export async function reconcileEntryImageStorage(
  ...
  for (const transaction of transactions) {
    ...
    for (const stagedFile of stagedFiles) {
      if (!stagedFile.isFile() || !generatedImageFileNamePattern.test(stagedFile.name)) {
        continue;
      }
      const imagePath = `${ENTRY_IMAGE_DIRECTORY}/${stagedFile.name}`;
  ...
  for (const entry of finalEntries) {
    ...
    const imagePath = `${ENTRY_IMAGE_DIRECTORY}/${entry.name}`;
    if (!ownedImagePaths.has(imagePath)) {
...
export async function stageNoteImages(
  ...
  assertEntrySaveLimits(contentMd, null, uploads);
  ...
  if (failed) {
    await discardStagedEntryImages(imagePaths, failed.reason);
  }
  ...
  try {
    assertEntrySaveLimits(updatedContent, null, uploads);
  } catch (error) {
    await discardStagedEntryImages(imagePaths, error);
  }
...
export async function stageEntryImages(
  ...
  assertEntrySaveLimits(notesMd, code, uploads);
  const staged = await stageNoteImages(notesMd, uploads);
  try {
    assertEntrySaveLimits(staged.contentMd, code, uploads);
  } catch (error) {
    await discardStagedEntryImages(staged.imagePaths, error);
  }
  return { notesMd: staged.contentMd, imagePaths: staged.imagePaths };
}

export function assertEntrySaveLimits(
  notesMd: string,
  code: string | null,
  uploads: ReadonlyMap<string, EntryImageUpload>,
): void {
  const notesBytes = utf8ByteLength(notesMd);
  if (notesBytes > ENTRY_NOTES_MAX_BYTES) {
    throw new ImageStorageError(
      "CONTENT_TOO_LARGE",
  ...
  const codeBytes = code === null ? 0 : utf8ByteLength(code);
  if (codeBytes > ENTRY_CODE_MAX_BYTES) {
    throw new ImageStorageError(
      "CODE_TOO_LARGE",
  ...
  let logicalBytes = notesBytes + codeBytes;
  for (const upload of uploads.values()) {
    assertDeclaredImageSize(upload.size);
    logicalBytes += upload.size;
    if (logicalBytes > ENTRY_SAVE_MAX_BYTES) {
      throw new ImageStorageError(
        "REQUEST_TOO_LARGE",
        "Markdown notes, code, and new images must not exceed 100 MiB in total.",
      );
    }
  }
}
...
async function discardStagedEntryImages(
  imagePaths: readonly string[],
  cause: unknown,
): Promise<never> {
...
export const reconcileNoteImageStorage = reconcileEntryImageStorage;
