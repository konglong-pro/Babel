...
import {
  ENTRY_UPLOAD_PLACEHOLDER_PREFIX,
    ...
    return withEntryMutationLock(async () => {
      ...
      try {
        const prepared = await prepareMarkdownFolderImport(root, sessionId, manifest, {
          ...
          uploadPlaceholderPrefix: ENTRY_UPLOAD_PLACEHOLDER_PREFIX,
        });
        for (const record of prepared.records) {
          ...
          const staged = await stageEntryImages(record.contentMd, uploads);
          allImagePaths.push(...staged.imagePaths);
          stagedRecords.push({ ...record, contentMd: staged.notesMd, imagePaths: staged.imagePaths });
        }
