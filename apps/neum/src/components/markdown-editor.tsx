...
export function MarkdownEditor({
  ...
  const autocomplete = useWikilinkAutocomplete(textareaRef, listEntryTitles, {
  ...
  function stageFiles(files: readonly File[], pasted: boolean) {
    ...
    for (const file of files) {
      ...
      accepted.push({
        image,
        markdown: `![${imageAlt(file, pasted)}](neum-upload://${token})`,
      });
      onStageImage(image);
    }
    ...
    <section className="editor-field">
      <div className="field-heading">
        <div>
          ...
          <p id={hintId}>
            Write Markdown with GFM tables, task lists, links, images, and [[entry links]].
          </p>
        </div>
