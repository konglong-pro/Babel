...
before(async () => {
  temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "leviathan-test-"));
  uploadDirectory = path.join(temporaryRoot, "uploads");
  process.env.LEVIATHAN_DATABASE_PATH = path.join(temporaryRoot, "sqlite.db");
  process.env.LEVIATHAN_UPLOAD_DIRECTORY = uploadDirectory;
...
test("Leviathan backend integration", async (t) => {
    ...
    await t.test("folder hierarchy prevents cycles and non-empty deletion", () => {
      const politics = repositories.listFolders()[0];
          ...
          body: noteForm(
            {
              ...
              title,
              contentMd: tokens
                .map((token) => `![${token}](leviathan-upload://${token})`)
                .join("\n"),
              tags: [],
            },
            images,
    ...
    await t.test("wire size is rejected before multipart parsing", async () => {
      ...
      for (const contentLength of [undefined, "1", "not-a-decimal-length"]) {
        const headers = new Headers({
          "Content-Type": "multipart/form-data; boundary=leviathan-test",
        });
      ...
      const body = multipartLike(
        {
          ...
          contentMd: "![image](leviathan-upload://image)",
          tags: [],
        },
        "image",
        {
    ...
    await t.test("note hierarchy rejects cycles, moves subtrees, and protects parents", async () => {
      const [politics, economics] = repositories.listFolders();
      ...
      assert.equal(movedLeaf.folderId, economics.id);

      const moved = repositories.updateNote(child.id, { folderId: economics.id }).note;
      assert.equal(moved.parentId, null);
      assert.equal(moved.folderId, economics.id);
      assert.equal(repositories.getNote(grandchild.id)?.folderId, economics.id);
    ...
    await t.test("notes support recursive listing, moving, normalized tags, and search", () => {
      ...
      const note = repositories.createNote({
        ...
        tags: [" Roman law ", "roman law", "Politics"],
      });
      assert.deepEqual(note.tags, ["Roman law", "Politics"]);
      ...
      assert.equal(moved.folderId, economics.id);
      assert.throws(
        () => repositories.deleteFolder(economics.id),
        (error: unknown) =>
          error instanceof repositories.RepositoryError && error.code === "NOT_EMPTY",
      );
    });
    ...
    await t.test("search API returns safe match details without full bodies", async () => {
      const folderId = repositories.listFolders()[0].id;
      const query = "leviathan-api-C++-needle";
      const note = repositories.createNote({
        folderId,
        title: "Leviathan API search contract",
    ...
    await t.test("partial concurrent image staging rolls back successful siblings", async () => {
      ...
      await assert.rejects(
        storage.stageNoteImages(
          "![good](leviathan-upload://good) ![bad](leviathan-upload://bad)",
          uploads,
        ),
        (error: unknown) =>
      ...
      await assert.rejects(
        storage.stageNoteImages("![missing](leviathan-upload://missing)", new Map()),
        (error: unknown) =>
          error instanceof storage.ImageStorageError && error.code === "INVALID_CONTENT",
      );
      assert.throws(
    ...
    await t.test("storage recovery restores owned quarantine and removes only generated orphans", async () => {
      ...
      const staged = await storage.stageNoteImages(
        "![Recovered](leviathan-upload://recovered)",
    ...
    await t.test("multipart API rolls back partial and database failures", async () => {
      const economics = repositories.listFolders().find(
        ({ name }) => name === "Economics",
      );
      assert.ok(economics);
      const folderId = economics.id;
      const before = await uploadFiles();
      ...
      const missingFolder = noteForm(
        {
          folderId: 999_999,
          title: "Rollback",
          contentMd: "![image](leviathan-upload://one)",
          tags: [],
        },
        [["one", png, "image/png"]],
      );
      ...
      const createForm = noteForm(
        {
          ...
          contentMd: "![Tiny](leviathan-upload://tiny)",
          tags: ["Image"],
        },
        [["tiny", png, "image/png"]],
      );
          ...
          body: noteForm(
            {
              ...
              contentMd: "![Delete](leviathan-upload://delete)",
              tags: [],
            },
