...
const NOTE_HEADING_ID_PREFIX = "leviathan-note-heading-";
const REMARK_FEATURES = ["gfm", "formula-math"] as const;
const PENDING_IMAGE_URL_PATTERN = /leviathan-upload:\/\/[A-Za-z0-9._-]+/g;
...
export function NoteDetail({
  ...
  const readerTriggerId = `leviathan-note-${detail?.id ?? draftKey}-reader-trigger`;
  ...
  if (!detail) {
    return (
      <section className="detail-panel empty-state" aria-label="Note details">
        ...
        <span className="empty-monogram" aria-hidden="true">L</span>
        <h2>Keep politics and economics close</h2>
      ...
      <header className="document-header">
        <div>
          <DetachedReaderWindow
            ...
            windowKey={`leviathan-note-${detail.id}`}
            buttonLabel="Read"
...
function NoteForm({
  ...
  const activeImportedReferences = useMemo(
    () => (importDraft?.imageReferences ?? []).filter((reference) =>
      content.includes(`leviathan-upload://${reference.token}`),
    ),
  ...
  const referencedStagedImages = stagedImages.filter((image) =>
    content.includes(`leviathan-upload://${image.token}`),
  );
  ...
  function changeContent(nextContent: string) {
    ...
    setStagedImages((current) => {
      const retained = current.filter((image) => {
        const referenced = nextContent.includes(`leviathan-upload://${image.token}`);
        ...
        <header className="document-header form-header">
          <div>
            <DetachedReaderWindow
              ...
              windowKey={`leviathan-note-${detail?.id ?? draftKey}`}
              buttonLabel="Read"
              buttonPortalTargetId={readerTriggerId}
              disabled={pending}
            >
        ...
        <div className="form-row form-columns">
          ...
          <label className="field tags-field">
            ...
            <input
              ...
              disabled={pending}
              value={tags}
              placeholder="political theory, institutions, markets"
              onChange={(event) => {
