...
export const NOTE_UPLOAD_PLACEHOLDER_PREFIX = "leviathan-upload://";
...
function uploadTokensInMarkdown(contentMd: string): Set<string> {
  ...
  const pattern = /leviathan-upload:\/\/([^\s)\]}'\"<>]+)/g;
...
function resolveNoteImageRoot(): string {
  const configuredUploadDirectory = process.env.LEVIATHAN_UPLOAD_DIRECTORY;
...
const markdownBaseUrl = new URL("https://leviathan.invalid/notes");
