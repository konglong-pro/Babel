...
export async function GET(): Promise<Response> {
  try {
    ...
    return Response.json({ status: "ok", app: "Leviathan" });
  } catch (error) {
    console.error("Leviathan health check failed", error);
    return Response.json(
      {
        error: {
          code: "SERVICE_UNAVAILABLE",
          message: "Leviathan storage is unavailable or requires migration.",
        },
      },
      { status: 503 },
    );
