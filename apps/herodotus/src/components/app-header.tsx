...
export const BEFORE_NAVIGATE_EVENT = "herodotus:before-navigate";
...
export function AppHeader() {
  ...
  function submitSearch(event: FormEvent<HTMLFormElement>) {
    ...
    if (!openSearchWindow(
      window,
      destination,
      "babel-herodotus-search",
      { sessionStorageKeys: ["babel:herodotus:pages"] },
    )) {
  ...
  return (
    <header className="app-header">
      <Link className="brand" href="/notes" aria-label="Herodotus notes home" onClick={visitHome}>
        <DavidPixelLogo className="brand-mark" />
        <span className="brand-copy">
          <strong>Herodotus</strong>
          <small>History &amp; literature</small>
        </span>
      </Link>
