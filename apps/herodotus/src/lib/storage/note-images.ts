...
export const NOTE_UPLOAD_PLACEHOLDER_PREFIX = "herodotus-upload://";
...
function uploadTokensInMarkdown(contentMd: string): Set<string> {
  ...
  const pattern = /herodotus-upload:\/\/([^\s)\]}'\"<>]+)/g;
...
function resolveNoteImageRoot(): string {
  const configuredUploadDirectory = process.env.HERODOTUS_UPLOAD_DIRECTORY;
...
const markdownBaseUrl = new URL("https://herodotus.invalid/notes");
