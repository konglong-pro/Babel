...
const latestMigration = 1_785_835_010_668;
const migrationsFolder = path.resolve(process.cwd(), "drizzle");

test("Herodotus database readiness", async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), "herodotus-readiness-"));
  ...
  await t.test("rejects a current database without folder ordering", () => {
  ...
  await t.test("health reports current and incompatible schemas", async () => {
    const originalPath = process.env.HERODOTUS_DATABASE_PATH;
    const originalUploads = process.env.HERODOTUS_UPLOAD_DIRECTORY;
    process.env.HERODOTUS_DATABASE_PATH = path.join(root, "current.db");
    process.env.HERODOTUS_UPLOAD_DIRECTORY = path.join(root, "uploads");
    await mkdir(process.env.HERODOTUS_UPLOAD_DIRECTORY, { recursive: true });
    assert.equal((await getHealth()).status, 200);
    const { sqlite } = await import("@/lib/db/client");
    sqlite.close();

    process.env.HERODOTUS_DATABASE_PATH = path.join(root, "missing-parent.db");
    const originalConsoleError = console.error;
    console.error = () => undefined;
    try {
      assert.equal((await getHealth()).status, 503);
      const missingPath = path.join(root, "missing.db");
      process.env.HERODOTUS_DATABASE_PATH = missingPath;
      assert.equal((await getHealth()).status, 503);
      assert.equal(existsSync(missingPath), false);
    } finally {
      console.error = originalConsoleError;
      if (originalPath === undefined) delete process.env.HERODOTUS_DATABASE_PATH;
      else process.env.HERODOTUS_DATABASE_PATH = originalPath;
      if (originalUploads === undefined) delete process.env.HERODOTUS_UPLOAD_DIRECTORY;
      else process.env.HERODOTUS_UPLOAD_DIRECTORY = originalUploads;
    }
  });
});
