...
import { reflectionPage } from "@/components/reflection-page-session";
...
export function NotesWorkspace({
  ...
  const showingTemplates = templateMode && activePage === null;
  ...
  return (
    ...
    >
      ...
      {showingTemplates ? (
        <TemplateEditor
          key={`template-${templateEditorVersion}-${selectedTemplateId ?? "new"}-${creatingTemplate}`}
        ...
        <>
          ...
          {activePage === null ? (
            <section className="detail-panel empty-state" data-babel-pane="detail" tabIndex={-1} aria-label="Note details">
              ...
              <span className="empty-monogram" aria-hidden="true">V</span>
              <h2>Keep several reflections in view</h2>
...
interface ActiveNotesHistoryGuardProps {
  readonly templateDirty: boolean;
