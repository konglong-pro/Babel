...
import { folders, noteImages, notes, reflections } from "@/lib/db/schema";
...
export function listMarkdownFolderImportTitles(): string[] {
  return [
    ...db.select({ title: notes.title }).from(notes).all(),
    ...db.select({ title: reflections.date }).from(reflections).all(),
  ].map(({ title }) => title);
}
...
export function importMarkdownFolderBatch(
  ...
  return db.transaction((transaction) => {
    ...
    assertUniqueImportTitles(
      records,
      [
        ...transaction.select({ title: notes.title }).from(notes).all(),
        ...transaction.select({ title: reflections.date }).from(reflections).all(),
      ].map(({ title }) => title),
    );
