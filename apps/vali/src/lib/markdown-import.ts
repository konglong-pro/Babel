...
/**
 * Decode one browser-selected Markdown file into an unsaved Vali draft.
...
> {
  ...
  for (const destination of [...inlineDestinations, ...referencedDestinations]) {
    ...
    replacements.push({
      ...
      value: `vali-upload://${reference.token}`,
    });
  }
...
function unsafeImagePath(path: string): MarkdownImportError {
  return new MarkdownImportError(
    "UNSAFE_IMAGE_PATH",
    `The image path 鈥?{path}鈥?is not a safe relative path.`,
    path,
  );
}
