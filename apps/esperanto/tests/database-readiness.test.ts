...
const latestMigration = 1_785_834_961_109;
const migrationsFolder = path.resolve(process.cwd(), "drizzle");

test("Esperanto database readiness", async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), "esperanto-readiness-"));
  ...
  await t.test("health reports current and incompatible schemas", async () => {
    ...
    try {
      ...
      process.env.ESPERANTO_DATABASE_PATH = missingPath;
      assert.equal((await getHealth()).status, 503);
