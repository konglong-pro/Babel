...
before(async () => {
  temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), "retex-search-test-"));
...
after(() => {
  ...
  assert.ok(resolved.startsWith(`${temporaryRoot}${path.sep}retex-search-test-`));
...
test("search tie breakers use field count, capped occurrences, position, updated time, and id", () => {
  ...
  const fieldQuery = "ReTex field count 8g4";
  ...
  const occurrenceQuery = "ReTex occurrence 4p2";
  ...
  const cappedQuery = "ReTex capped count 2k6";
  ...
  const stableQuery = "ReTex stable order 5m1";
  ...
  const emojiQuery = "ReTex emoji position 9q3";
