...
test("legacy exercise images migrate into managed problem Markdown without data loss", async () => {
  const temporaryDirectory = await mkdtemp(
    path.join(os.tmpdir(), "retex-exercise-problem-migration-"),
  );
  const legacyRoot = path.join(temporaryDirectory, "uploads", "exercises");
  const noteRoot = path.join(temporaryDirectory, "uploads", "notes");
  const previousLegacyRoot = process.env.RETEX_UPLOAD_DIRECTORY;
  const previousNoteRoot = process.env.RETEX_NOTE_UPLOAD_DIRECTORY;
  process.env.RETEX_UPLOAD_DIRECTORY = legacyRoot;
  process.env.RETEX_NOTE_UPLOAD_DIRECTORY = noteRoot;
  ...
  try {
    ...
    assert.equal(
      sqlite
        ...
        .pluck()
        .get("data/retex/uploads/notes/legacy-exercise-7-problem.png"),
      1,
    );
    assert.equal(
    ...
    sqlite.close();
    restoreEnvironment("RETEX_UPLOAD_DIRECTORY", previousLegacyRoot);
    restoreEnvironment("RETEX_NOTE_UPLOAD_DIRECTORY", previousNoteRoot);
    const resolved = path.resolve(temporaryDirectory);
    assert.ok(
      resolved.startsWith(
        `${path.resolve(os.tmpdir())}${path.sep}retex-exercise-problem-migration-`,
      ),
    );
...
test("legacy migration refuses to alter the database when an image is missing", async () => {
  const temporaryDirectory = await mkdtemp(
    path.join(os.tmpdir(), "retex-exercise-problem-missing-"),
  );
  const previousLegacyRoot = process.env.RETEX_UPLOAD_DIRECTORY;
  const previousNoteRoot = process.env.RETEX_NOTE_UPLOAD_DIRECTORY;
  process.env.RETEX_UPLOAD_DIRECTORY = path.join(temporaryDirectory, "legacy");
  process.env.RETEX_NOTE_UPLOAD_DIRECTORY = path.join(temporaryDirectory, "notes");
  const sqlite = new BetterSqlite3(path.join(temporaryDirectory, "sqlite.db"));
