...
test("search opens in its own reusable window and clears cloned page sessions", () => {
  ...
  assert.equal(
    openSearchWindow(source, "/search?q=target", "babel-retex-search", {
  ...
  assert.deepEqual(opened, {
    ...
    name: "babel-retex-search",
  });
  assert.deepEqual(removedStorageKeys, ["babel:retex:pages"]);

  const blocked = { open: () => null } as unknown as Pick<Window, "open">;
  assert.equal(openSearchWindow(blocked, "/search?q=target", "babel-retex-search"), false);
});
