...
export const NOTE_IMAGE_DIRECTORY = "data/retex/uploads/notes";
export const NOTE_UPLOAD_PLACEHOLDER_PREFIX = "retex-upload://";
...
function uploadTokensInMarkdown(contentMd: string): Set<string> {
  const tokens = new Set<string>();
  const pattern = /retex-upload:\/\/([^\s)\]}'"<>]+)/g;
...
function resolveNoteImageRoot(): string {
  const configuredUploadDirectory = process.env.RETEX_NOTE_UPLOAD_DIRECTORY;
...
const markdownBaseUrl = new URL("https://retex.invalid/notes");
