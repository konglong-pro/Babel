...
const noteLogicalPrefix = "data/retex/uploads/notes/";
...
const migratedLogicalPathPattern =
  /^data\/retex\/uploads\/notes\/(legacy-exercise-(\d+)-([A-Za-z0-9._-]+))$/;
...
function resolveLegacyImageRoot(): string {
  return process.env.RETEX_UPLOAD_DIRECTORY === undefined
    ? path.resolve(process.cwd(), "..", "..", "data", "retex", "uploads", "exercises")
    : path.resolve(process.env.RETEX_UPLOAD_DIRECTORY);
}

function resolveNoteImageRoot(): string {
  return process.env.RETEX_NOTE_UPLOAD_DIRECTORY === undefined
    ? path.resolve(process.cwd(), "..", "..", "data", "retex", "uploads", "notes")
    : path.resolve(process.env.RETEX_NOTE_UPLOAD_DIRECTORY);
}
