...
const previousDatabasePath = process.env.RETEX_DATABASE_PATH;
const previousUploadDirectory = process.env.RETEX_NOTE_UPLOAD_DIRECTORY;
const temporaryDirectory = await mkdtemp(
  path.join(os.tmpdir(), "babel-retex-search-benchmark-"),
);
const databasePath = path.join(temporaryDirectory, "sqlite.db");
let closeDatabase: (() => void) | undefined;

try {
  process.env.RETEX_DATABASE_PATH = databasePath;
  ...
  restoreEnvironment("RETEX_NOTE_UPLOAD_DIRECTORY", previousUploadDirectory);
}
}
