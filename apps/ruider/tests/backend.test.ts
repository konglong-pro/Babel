...
before(async () => {
  temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "ruider-test-"));
  uploadDirectory = path.join(temporaryRoot, "uploads");
  process.env.RUIDER_DATABASE_PATH = path.join(temporaryRoot, "sqlite.db");
  process.env.RUIDER_UPLOAD_DIRECTORY = uploadDirectory;
...
test("Ruider backend integration", async (t) => {
    await t.test("migration seeds the three editable root folders in order", () => {
      assert.deepEqual(
        ...
        [
          { name: "Vocabulary", parentId: null, position: 0 },
          { name: "Grammar", parentId: null, position: 1 },
          { name: "Expressions", parentId: null, position: 2 },
        ],
      );
    });
          ...
          body: noteForm(
            {
              ...
              title,
              contentMd: tokens
                .map((token) => `![${token}](ruider-upload://${token})`)
                .join("\n"),
              tags: [],
            },
            images,
    ...
    await t.test("wire size is rejected before multipart parsing", async () => {
      ...
      for (const contentLength of [undefined, "1", "not-a-decimal-length"]) {
        const headers = new Headers({
          "Content-Type": "multipart/form-data; boundary=ruider-test",
        });
      ...
      const body = multipartLike(
        {
          ...
          contentMd: "![image](ruider-upload://image)",
          tags: [],
        },
        "image",
        {
    ...
    await t.test("search paginates globally ranked results and keeps its index synchronized", () => {
      const folderId = repositories.listFolders()[0].id;
      const query = "ruider-page-index-needle";
    ...
    await t.test("search API returns safe match details without full bodies", async () => {
      const folderId = repositories.listFolders()[0].id;
      const query = "mirror-api-C++-needle";
      const note = repositories.createNote({
        folderId,
        title: "Mirror API search contract",
    ...
    await t.test("partial concurrent image staging rolls back successful siblings", async () => {
      ...
      await assert.rejects(
        storage.stageNoteImages(
          "![good](ruider-upload://good) ![bad](ruider-upload://bad)",
          uploads,
        ),
        (error: unknown) =>
      ...
      await assert.rejects(
        storage.stageNoteImages("![missing](ruider-upload://missing)", new Map()),
        (error: unknown) =>
          error instanceof storage.ImageStorageError && error.code === "INVALID_CONTENT",
      );
      assert.throws(
    ...
    await t.test("storage recovery restores owned quarantine and removes only generated orphans", async () => {
      ...
      const staged = await storage.stageNoteImages(
        "![Recovered](ruider-upload://recovered)",
      ...
      const missingFolder = noteForm(
        {
          folderId: 999_999,
          title: "Rollback",
          contentMd: "![image](ruider-upload://one)",
          tags: [],
        },
        [["one", png, "image/png"]],
      );
      ...
      const createForm = noteForm(
        {
          ...
          contentMd: "![Tiny](ruider-upload://tiny)",
          tags: ["Image"],
        },
        [["tiny", png, "image/png"]],
      );
          ...
          body: noteForm(
            {
              ...
              contentMd: "![Delete](ruider-upload://delete)",
              tags: [],
            },
