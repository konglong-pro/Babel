...
test("inline and reference images are rewritten while remote images remain unchanged", () => {
  ...
  assert.equal(
    imported.contentMd.match(new RegExp(`ruider-upload:\\/\\/${map.token}`, "g"))?.length,
    2,
  );
  assert.match(imported.contentMd, /\[cover\]: ruider-upload:\/\/import-[a-f0-9]+ "Cover"/);
...
test("the same normalized local path receives one stable token", () => {
  ...
  assert.equal(
    first.contentMd.match(/ruider-upload:\/\/import-[a-f0-9]+/g)?.length,
    2,
  );
});
...
test("an indented list continuation imports an image rendered by GFM", () => {
  ...
  assert.match(imported.contentMd, /ruider-upload:\/\/import-[a-f0-9]+/);
});
...
test("a blockquote image reference rewrites only its definition destination", () => {
  ...
  assert.match(
    imported.contentMd,
    /^> \[map\]: ruider-upload:\/\/import-[a-f0-9]+$/m,
  );
});
...
test("an unclosed blockquote fence ends when its container ends", () => {
  ...
  assert.match(imported.contentMd, /outside !\[Image\]\(ruider-upload:\/\//);
});
...
test("reference definitions work in repeated blockquotes and list containers", () => {
  ...
  assert.match(imported.contentMd, /^>>   \[deep\]: ruider-upload:\/\//m);
  assert.match(imported.contentMd, /^     \[plate\]: ruider-upload:\/\//m);
});
