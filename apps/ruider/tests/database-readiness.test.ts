...
const latestMigration = 1_785_835_739_539;
const migrationsFolder = path.resolve(process.cwd(), "drizzle");

test("Ruider database readiness", async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), "ruider-readiness-"));
  ...
  await t.test("health reports current and incompatible schemas", async () => {
    const originalPath = process.env.RUIDER_DATABASE_PATH;
    const originalUploads = process.env.RUIDER_UPLOAD_DIRECTORY;
    process.env.RUIDER_DATABASE_PATH = path.join(root, "current.db");
    process.env.RUIDER_UPLOAD_DIRECTORY = path.join(root, "uploads");
    await mkdir(process.env.RUIDER_UPLOAD_DIRECTORY, { recursive: true });
    assert.equal((await getHealth()).status, 200);
    const { sqlite } = await import("@/lib/db/client");
    sqlite.close();

    process.env.RUIDER_DATABASE_PATH = path.join(root, "missing-parent.db");
    const originalConsoleError = console.error;
    console.error = () => undefined;
    try {
      assert.equal((await getHealth()).status, 503);
      const missingPath = path.join(root, "missing.db");
      process.env.RUIDER_DATABASE_PATH = missingPath;
      assert.equal((await getHealth()).status, 503);
      assert.equal(existsSync(missingPath), false);
    } finally {
      console.error = originalConsoleError;
      if (originalPath === undefined) delete process.env.RUIDER_DATABASE_PATH;
      else process.env.RUIDER_DATABASE_PATH = originalPath;
      if (originalUploads === undefined) delete process.env.RUIDER_UPLOAD_DIRECTORY;
      else process.env.RUIDER_UPLOAD_DIRECTORY = originalUploads;
    }
  });
});
