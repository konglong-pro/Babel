...
export function CanvasEditor({
  ...
  function handleStagePointerDown(event: PointerEvent<SVGSVGElement>) {
    ...
    if (tool === "pen") {
      element = { id, type: "path", points: [point, point] };
    } else if (tool === "arrow") {
      element = { id, type: "arrow", start: point, end: point };
    } else {
      element = {
        id,
        type: "shape",
      ...
      setScene((current) => ({
        ...
        elements: current.elements.map((element) =>
          element.id === interaction.elementId
            ? updateDraftElement(element, interaction.origin, point)
            : element,
        ),
      }));
      return;
  ...
  function handleWheel(event: WheelEvent<SVGSVGElement>) {
    ...
    const zoom = clamp(scene.viewport.zoom * factor, 0.1, 4);
    setScene({
      ...scene,
      viewport: {
  ...
  function setZoom(zoom: number) {
    ...
    setScene({
      ...scene,
      viewport: { x: center.x - world.x * nextZoom, y: center.y - world.y * nextZoom, zoom: nextZoom },
    });
  }
  ...
  function updateCardText(id: string, text: string) {
    setScene((current) => ({
      ...current,
      elements: current.elements.map((element) =>
        element.id === id && element.type === "card" ? { ...element, text } : element,
      ),
    }));
  }

  const gridSize = 24 * scene.viewport.zoom;
    ...
    <div className="canvas-editor">
      ...
      <div className="canvas-stage-wrap">
        <svg
          ...
          role="application"
          aria-label="Infinite brainstorming canvas"
          tabIndex={0}
        ...
        >
          <defs>
            <pattern
              ...
              x={scene.viewport.x % gridSize}
              y={scene.viewport.y % gridSize}
            >
              <circle cx={1} cy={1} r={1} className="canvas-grid-dot" />
            </pattern>
            <marker id="ruider-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
              <path d="M 0 0 L 10 5 L 0 10 z" />
            </marker>
          </defs>
          <rect width="100%" height="100%" className="canvas-grid" />
          <g transform={`translate(${scene.viewport.x} ${scene.viewport.y}) scale(${scene.viewport.zoom})`}>
            {scene.elements.map((element) => (
              <CanvasElementView
                ...
                onCardTextChange={(text) => updateCardText(element.id, text)}
              />
            ))}
          </g>
        </svg>
...
function CanvasElementView({ element, selected, onPointerDown, onCardTextChange }: CanvasElementViewProps) {
      ...
      <g className={className} onPointerDown={onPointerDown}>
        ...
        <foreignObject x={element.x + 10} y={element.y + 30} width={element.width - 20} height={element.height - 40}>
          <textarea
            ...
            maxLength={20_000}
            placeholder="Type an unfinished thought…"
            value={element.text}
  ...
  if (element.type === "path") {
    ...
    return (
      <g className={className} onPointerDown={onPointerDown}>
        <polyline className="canvas-hit-stroke" points={points} />
        <polyline className="canvas-path" points={points} />
      </g>
    );
  }
  if (element.type === "arrow") {
    return (
      <g className={className} onPointerDown={onPointerDown}>
        ...
        <line className="canvas-arrow" x1={element.start.x} y1={element.start.y} x2={element.end.x} y2={element.end.y} markerEnd="url(#ruider-arrow)" />
      </g>
    );
  }
...
function updateDraftElement(element: CanvasElement, origin: CanvasPoint, point: CanvasPoint): CanvasElement {
  ...
  if (element.type === "arrow") {
    return { ...element, end: point } satisfies CanvasArrowElement;
  }
