...
export const NOTE_UPLOAD_PLACEHOLDER_PREFIX = "ruider-upload://";
...
function uploadTokensInMarkdown(contentMd: string): Set<string> {
  ...
  const pattern = /ruider-upload:\/\/([^\s)\]}'\"<>]+)/g;
...
function resolveNoteImageRoot(): string {
  const configuredUploadDirectory = process.env.RUIDER_UPLOAD_DIRECTORY;
...
const markdownBaseUrl = new URL("https://ruider.invalid/notes");
