...
before(async () => {
  root = await mkdtemp(path.join(os.tmpdir(), "klsche-templates-test-"));
  process.env.KLSCHE_DATABASE_PATH = path.join(root, "sqlite.db");
...
test("KLsche draft templates are independent, bounded static Markdown", async () => {
  const contentMd = "# Scene\n\n## Fragments\n\n- Opening image";
  ...
  for (const forbidden of [
    "![Pending](klsche-upload://draft-image)",
  ...
  const draft = repositories.createNote({
    folderId: repositories.listFolders()[0].id,
    title: "Copied opening",
    contentMd,
  });
  const updated = await item.PATCH(
    jsonRequest("PATCH", { name: "Revised Scene", contentMd: "# Changed" }, created.id),
    routeContext(created.id),
  );
  assert.equal(updated.status, 200);
  assert.equal(repositories.getNote(draft.id)?.contentMd, contentMd);
