...
before(async () => {
  temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "klsche-test-"));
  uploadDirectory = path.join(temporaryRoot, "uploads");
  process.env.KLSCHE_DATABASE_PATH = path.join(temporaryRoot, "sqlite.db");
  process.env.KLSCHE_UPLOAD_DIRECTORY = uploadDirectory;
...
test("KLsche backend integration", async (t) => {
    await t.test("migration seeds the three editable root folders in order", () => {
      assert.deepEqual(
        ...
        [
          { name: "Fragments", parentId: null, position: 0 },
          { name: "Drafts", parentId: null, position: 1 },
          { name: "Experiments", parentId: null, position: 2 },
        ],
      );
    });
          ...
          body: noteForm(
            {
              ...
              title,
              contentMd: tokens
                .map((token) => `![${token}](klsche-upload://${token})`)
                .join("\n"),
              tags: [],
            },
            images,
    ...
    await t.test("wire size is rejected before multipart parsing", async () => {
      ...
      for (const contentLength of [undefined, "1", "not-a-decimal-length"]) {
        const headers = new Headers({
          "Content-Type": "multipart/form-data; boundary=klsche-test",
        });
      ...
      const body = multipartLike(
        {
          ...
          contentMd: "![image](klsche-upload://image)",
          tags: [],
        },
        "image",
        {
    ...
    await t.test("search paginates globally ranked results and keeps its index synchronized", () => {
      const folderId = repositories.listFolders()[0].id;
      const query = "klsche-page-index-needle";
    ...
    await t.test("partial concurrent image staging rolls back successful siblings", async () => {
      ...
      await assert.rejects(
        storage.stageNoteImages(
          "![good](klsche-upload://good) ![bad](klsche-upload://bad)",
          uploads,
        ),
        (error: unknown) =>
      ...
      await assert.rejects(
        storage.stageNoteImages("![missing](klsche-upload://missing)", new Map()),
        (error: unknown) =>
          error instanceof storage.ImageStorageError && error.code === "INVALID_CONTENT",
      );
      assert.throws(
    ...
    await t.test("storage recovery restores owned quarantine and removes only generated orphans", async () => {
      ...
      const staged = await storage.stageNoteImages(
        "![Recovered](klsche-upload://recovered)",
      ...
      const missingFolder = noteForm(
        {
          folderId: 999_999,
          title: "Rollback",
          contentMd: "![image](klsche-upload://one)",
          tags: [],
        },
        [["one", png, "image/png"]],
      );
      ...
      const createForm = noteForm(
        {
          ...
          contentMd: "![Tiny](klsche-upload://tiny)",
          tags: ["Image"],
        },
        [["tiny", png, "image/png"]],
      );
          ...
          body: noteForm(
            {
              ...
              contentMd: "![Delete](klsche-upload://delete)",
              tags: [],
            },
