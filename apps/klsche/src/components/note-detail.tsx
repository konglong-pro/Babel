...
const NOTE_HEADING_ID_PREFIX = "klsche-note-heading-";
const REMARK_FEATURES = ["gfm", "formula-math"] as const;
const PENDING_IMAGE_URL_PATTERN = /klsche-upload:\/\/[A-Za-z0-9._-]+/g;
...
function NoteReaderDraft({
  ...
  return (
    <article
      ...
      aria-label={liveDraft ? "Live draft reader" : "Draft reader"}
    >
      <header className="document-header">
        <div>
          <span className="eyebrow">
            {folderLabel || (liveDraft ? "Work in progress" : "Finished draft")}
          </span>
          <h1>{title.trim() || "Untitled draft"}</h1>
          <Tags tags={tags} />
          {liveDraft ? (
...
export function NoteDetail({
  ...
  const readerTriggerId = `klsche-note-${detail?.id ?? draftKey}-reader-trigger`;
  ...
  if (!detail) {
    return (
      <section className="detail-panel empty-state" data-babel-pane="detail" tabIndex={-1} aria-label="Note details">
        ...
        <h2>Turn fragments into form</h2>
        <p>Choose a draft, or open a project and begin with a line.</p>
      </section>
    );
  }
  ...
  return (
    <article ref={detailRootRef} className="detail-panel document-view" data-babel-pane="detail" tabIndex={-1}>
      ...
      <header className="document-header">
        <div>
          <DetachedReaderWindow
            ...
            windowKey={`klsche-note-${detail.id}`}
            buttonLabel="Read"
      ...
      <section className="linked-mentions" aria-labelledby="linked-mentions-heading">
        ...
        {backlinks.length === 0 ? (
          <p className="empty-copy">No drafts link here yet.</p>
        ) : (
          <ul>
...
function NoteForm({
  ...
  const activeImportedReferences = useMemo(
    () => (importDraft?.imageReferences ?? []).filter((reference) =>
      content.includes(`klsche-upload://${reference.token}`),
    ),
  ...
  const referencedStagedImages = stagedImages.filter((image) =>
    content.includes(`klsche-upload://${image.token}`),
  );
  ...
  function changeContent(nextContent: string) {
    ...
    setStagedImages((current) => {
      const retained = current.filter((image) => {
        const referenced = nextContent.includes(`klsche-upload://${image.token}`);
    ...
    <section className="detail-panel form-view" data-babel-pane="detail" tabIndex={-1}>
      <form ref={formRef} onSubmit={submit}>
        <header className="document-header form-header">
          <div>
            <DetachedReaderWindow
              title={`${title.trim() || "Untitled draft"} - Reader`}
              windowKey={`klsche-note-${detail?.id ?? draftKey}`}
              buttonLabel="Read"
              buttonPortalTargetId={readerTriggerId}
              disabled={pending}
            >
            ...
            <span className="eyebrow">
              {detail ? "Revise the draft" : importDraft ? "Bring in a fragment" : "Start a draft"}
            </span>
            <h1>
              {detail ? detail.title : importDraft ? importDraft.title : "Begin with a fragment"}
            </h1>
          </div>
          <div className="document-actions">
        ...
        <div className="form-row form-columns">
          ...
          <label className="field title-field">
            ...
            <input
              ...
              disabled={pending}
              value={title}
              placeholder="A title that holds the idea"
              onChange={(event) => {
        ...
        {detail ? (
          <DetachedEditorWindow
            title={`${title.trim() || "Untitled draft"} - Editor`}
            windowKey={`klsche-note-${detail.id}`}
            disabled={pending}
