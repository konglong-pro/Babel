...
export function SearchResults({ query }: { query: string }) {
  ...
  useCommandPaletteActions("klsche.search", canLoadMore ? [
    {
      id: "search.loadMore",
  ...
  return (
    <div className="search-page">
      <header className="search-heading">
        <span className="eyebrow">Search the studio</span>
        <h1>{query ? `Results for “${query}”` : "Find a draft"}</h1>
        <p>Search across titles, fragments, and tags.</p>
      </header>
      ...
      {!loading && !query && !error ? (
        <div className="empty-state search-empty">
          ...
          <p>Try a word, phrase, or tag from your drafts.</p>
        </div>
      ) : null}
