...
export function NoteList({
  ...
  useCommandPaletteActions("klsche.notes", [
    {
      id: "note.new",
      label: "New note",
    ...
    >
      ...
      <div className="panel-heading note-list-heading">
        <div>
          ...
          <h2>{selectedFolder?.name ?? "All Drafts"}</h2>
          <p>{notes.length} {notes.length === 1 ? "draft" : "drafts"}</p>
        </div>
      ...
      {selectedFolderId === null ? (
        <p className="panel-hint" id={folderActionHintId}>
          Choose a project to start a draft. All Drafts remains a read-only overview.
        </p>
      ) : null}
      {loading ? <p className="panel-status">Gathering drafts…</p> : null}
      {!loading && notes.length === 0 ? (
        <div className="empty-state compact-empty">
          <span aria-hidden="true">D</span>
          <h3>No drafts yet</h3>
          <p>{selectedFolderId === null ? "The first fragment starts here." : "Start a draft in this project when the idea arrives."}</p>
        </div>
      ) : null}
