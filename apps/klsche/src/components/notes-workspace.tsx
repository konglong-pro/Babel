...
export function NotesWorkspace({
  ...
  const activePage = pages.find(
    (page) => page.key === activeKey && page.kind === PAGE_KIND,
  ) ?? null;
  useEffect(() => {
  ...
  return (
    ...
    >
        ...
        <NoteList
          ...
          onCreate={(parentId) => {
            ...
            openDraft({
              ...
              parentId,
              importDraft: null,
              title: parentId === null ? "New draft" : "New subdraft",
            });
          }}
          onBack={() => {
            activatePage(null);
      ...
      {showingTemplates ? (
        ...
        <>
          {pages
            .filter((page) => page.kind === PAGE_KIND)
            .map((page) => {
          ...
          {activePage === null ? (
            <section className="detail-panel empty-state" data-babel-pane="detail" tabIndex={-1} aria-label="Draft details">
              <button className="content-back" type="button" onClick={() => showList()}>
                <span aria-hidden="true">←</span> Drafts
              </button>
              <span className="empty-monogram" aria-hidden="true">K</span>
              <h2>Keep several drafts in motion</h2>
              <p>Select a draft to open it in a persistent page tab.</p>
            </section>
          ) : null}
        </>
      )}
