...
const HISTORY_GUARD_KEY = "__klscheDirtyGuard";
...
export function NotesWorkspace({
  ...
  const confirmDiscard = useCallback((): boolean => {
    if (savingRef.current || wikilinkCreatePendingRef.current) {
      window.alert("Please wait for the draft to finish saving.");
      return false;
    }
    ...
    <div className={`notes-workspace stage-${stage}${dirty ? " has-unsaved" : ""}`}>
      ...
      {detailError ? (
        <section className="detail-panel error-state" role="alert">
          <button className="content-back" type="button" onClick={backToNotes}>← Drafts</button>
