...
export function NotePageSession({
  ...
  useCommandPaletteActions(`klsche.note-detail.${pageKey}`, activeKey === pageKey && Boolean(loadError) ? [
    {
      id: "note.retry",
    ...
    return (
      <PageDeckPage pageKey={pageKey}>
        <section className="detail-panel error-state" data-babel-pane="detail" tabIndex={-1} role="alert">
          <button className="content-back" data-babel-escape="list" type="button" onClick={onShowList}>← Drafts</button>
          <span aria-hidden="true">!</span>
          <h2>Could not load this draft</h2>
          <p>{loadError}</p>
