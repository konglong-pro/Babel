...
export function FolderPanel({
  ...
  const revealedTreeState = revealFolderSelection(treeState, selectedPath);
  ...
  useCommandPaletteActions("klsche.folders", [
    {
      id: "folder.new",
  ...
  function toggleFolder(folderId: number) {
    setTreeState((current) => {
      ...
      return {
        ...
        revealedPathKey: revealedState.revealedPathKey,
      };
    });
  }
  ...
  return (
    ...
    >
      ...
      <nav className="folder-tree" {...navigation.treeProps}>
        ...
        >
          <span className="all-notes-glyph" aria-hidden="true">D</span>
      ...
      <dialog ref={dialogRef} className="dialog" aria-labelledby={dialogTitleId}>
        <form className="dialog-body" onSubmit={submit}>
          <p className="eyebrow">Shape the studio</p>
          <h2 id={dialogTitleId}>
          ...
          {dialogMode === "move" ? (
            <label className="field">
              ...
              <select name="parentId" value={targetId} onChange={(event) => setTargetId(event.target.value)}>
                <option value="">Studio root</option>
