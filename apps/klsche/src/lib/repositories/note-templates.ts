...
export function updateNoteTemplate(
  ...
  if (input.name !== undefined) {
    ...
    assertTemplateNameLength(changes.name);
...
function assertTemplateNameLength(name: string): void {
  if (name.length > TEMPLATE_NAME_MAX_LENGTH) {
    throw new RepositoryError(
      "VALIDATION",
      `Template names must not exceed ${TEMPLATE_NAME_MAX_LENGTH} characters.`,
...
function normalizeTemplateMarkdown(value: unknown): string {
  ...
  if (
    /klsche-upload:\/\//iu.test(contentMd) ||
    managedImagePathsInMarkdown(contentMd).size > 0
  ) {
    throw new RepositoryError(
      "VALIDATION",
      "Templates cannot contain managed draft images.",
      { field: "contentMd" },
    );
  }
  return contentMd;
