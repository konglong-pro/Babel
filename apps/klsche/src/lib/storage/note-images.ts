...
export const NOTE_UPLOAD_PLACEHOLDER_PREFIX = "klsche-upload://";
...
function uploadTokensInMarkdown(contentMd: string): Set<string> {
  ...
  const pattern = /klsche-upload:\/\/([^\s)\]}'\"<>]+)/g;
...
function resolveNoteImageRoot(): string {
  const configuredUploadDirectory = process.env.KLSCHE_UPLOAD_DIRECTORY;
...
const markdownBaseUrl = new URL("https://klsche.invalid/notes");
