...
Bio is Babel's local, single-user biology notebook. It organizes Markdown
...
```powershell
npm.cmd run dev:bio
npm.cmd run check -w @babel-apps/bio
```

Open `http://127.0.0.1:3009`. Private data lives in
`data/bio/sqlite.db` and `data/bio/uploads/notes/` in the independent
private `data/` repository. The launcher supplies absolute
`BIO_DATABASE_PATH` and `BIO_UPLOAD_DIRECTORY` values.
...
Bio follows Babel's Ready/Edit keyboard model. `Ctrl+F6` and
...
while Bio is stopped. Use the root `npm.cmd run data:backup` workflow for
