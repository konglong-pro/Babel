...
before(async () => {
  temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "bio-test-"));
  uploadDirectory = path.join(temporaryRoot, "uploads");
  process.env.BIO_DATABASE_PATH = path.join(temporaryRoot, "sqlite.db");
  process.env.BIO_UPLOAD_DIRECTORY = uploadDirectory;
...
test("Bio backend integration", async (t) => {
          ...
          body: noteForm(
            {
              ...
              title,
              contentMd: tokens
                .map((token) => `![${token}](bio-upload://${token})`)
                .join("\n"),
              tags: [],
            },
            images,
    ...
    await t.test("wire size is rejected before multipart parsing", async () => {
      ...
      for (const contentLength of [undefined, "1", "not-a-decimal-length"]) {
        const headers = new Headers({
          "Content-Type": "multipart/form-data; boundary=bio-test",
        });
      ...
      const body = multipartLike(
        {
          ...
          contentMd: "![image](bio-upload://image)",
          tags: [],
        },
        "image",
        {
    ...
    await t.test("search paginates globally ranked results and keeps its index synchronized", () => {
      const folderId = repositories.listFolders()[0].id;
      const query = "bio-page-index-needle";
    ...
    await t.test("partial concurrent image staging rolls back successful siblings", async () => {
      ...
      await assert.rejects(
        storage.stageNoteImages(
          "![good](bio-upload://good) ![bad](bio-upload://bad)",
          uploads,
        ),
        (error: unknown) =>
      ...
      await assert.rejects(
        storage.stageNoteImages("![missing](bio-upload://missing)", new Map()),
        (error: unknown) =>
          error instanceof storage.ImageStorageError && error.code === "INVALID_CONTENT",
      );
      assert.throws(
    ...
    await t.test("storage recovery restores owned quarantine and removes only generated orphans", async () => {
      ...
      const staged = await storage.stageNoteImages(
        "![Recovered](bio-upload://recovered)",
      ...
      const missingFolder = noteForm(
        {
          folderId: 999_999,
          title: "Rollback",
          contentMd: "![image](bio-upload://one)",
          tags: [],
        },
        [["one", png, "image/png"]],
      );
      ...
      const createForm = noteForm(
        {
          ...
          contentMd: "![Tiny](bio-upload://tiny)",
          tags: ["Image"],
        },
        [["tiny", png, "image/png"]],
      );
          ...
          body: noteForm(
            {
              ...
              contentMd: "![Delete](bio-upload://delete)",
              tags: [],
            },
