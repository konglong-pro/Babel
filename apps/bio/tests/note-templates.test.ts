...
before(async () => {
  temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "bio-templates-test-"));
  process.env.BIO_DATABASE_PATH = path.join(temporaryRoot, "sqlite.db");
  process.env.BIO_UPLOAD_DIRECTORY = path.join(temporaryRoot, "uploads");
...
test("note template CRUD and copy semantics", async (t) => {
  ...
  await t.test("managed note images are rejected", async () => {
    for (const contentMd of [
      "![Pending](bio-upload://image-token)",
