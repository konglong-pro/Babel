...
const NOTE_HEADING_ID_PREFIX = "bio-note-heading-";
const REMARK_FEATURES = ["gfm", "formula-math"] as const;
const PENDING_IMAGE_URL_PATTERN = /bio-upload:\/\/[A-Za-z0-9._-]+/g;
...
export function NoteDetail({
  ...
  const readerTriggerId = `bio-note-${detail?.id ?? draftKey}-reader-trigger`;
  ...
  if (!detail) {
    return (
      <section className="detail-panel empty-state" aria-label="Note details">
        ...
        <span className="empty-monogram" aria-hidden="true">B</span>
        <h2>Build your biology notebook</h2>
...
function NoteForm({
  ...
  const activeImportedReferences = useMemo(
    () => (importDraft?.imageReferences ?? []).filter((reference) =>
      content.includes(`bio-upload://${reference.token}`),
    ),
  ...
  const referencedStagedImages = stagedImages.filter((image) =>
    content.includes(`bio-upload://${image.token}`),
  );
  ...
  function changeContent(nextContent: string) {
    ...
    setStagedImages((current) => {
      const retained = current.filter((image) => {
        const referenced = nextContent.includes(`bio-upload://${image.token}`);
        ...
        <header className="document-header form-header">
          <div>
            <DetachedReaderWindow
              ...
              windowKey={`bio-note-${detail?.id ?? draftKey}`}
              buttonLabel="Read"
              buttonPortalTargetId={readerTriggerId}
              disabled={pending}
            >
