...
import { CloneSheepLogo } from "@/components/clone-sheep-logo";

export const BEFORE_NAVIGATE_EVENT = "bio:before-navigate";
...
export function AppHeader() {
  ...
  function submitSearch(event: FormEvent<HTMLFormElement>) {
    ...
    if (!openSearchWindow(
      ...
      destination,
      "babel-bio-search",
      { sessionStorageKeys: ["babel:bio:pages"] },
    )) {
  ...
  return (
    <header className="app-header">
      <Link className="brand" href="/notes" aria-label="Bio notes home" onClick={visitHome}>
        <span className="clone-sheep-mark" aria-hidden="true">
          <CloneSheepLogo />
        </span>
        <span className="brand-copy">
          <strong>Bio</strong>
          <small>Life / Archive 01</small>
        </span>
      </Link>
