...
test("inline and reference images are rewritten while remote images remain unchanged", () => {
  ...
  assert.equal(
    imported.contentMd.match(new RegExp(`__APP_ID__-upload:\\/\\/${map.token}`, "g"))?.length,
    2,
  );
  assert.match(imported.contentMd, /\[cover\]: __APP_ID__-upload:\/\/import-[a-f0-9]+ "Cover"/);
...
test("the same normalized local path receives one stable token", () => {
  ...
  assert.equal(
    first.contentMd.match(/__APP_ID__-upload:\/\/import-[a-f0-9]+/g)?.length,
    2,
  );
});
...
test("an indented list continuation imports an image rendered by GFM", () => {
  ...
  assert.match(imported.contentMd, /__APP_ID__-upload:\/\/import-[a-f0-9]+/);
});
...
test("a blockquote image reference rewrites only its definition destination", () => {
  ...
  assert.match(
    imported.contentMd,
    /^> \[map\]: __APP_ID__-upload:\/\/import-[a-f0-9]+$/m,
  );
});
...
test("an unclosed blockquote fence ends when its container ends", () => {
  ...
  assert.match(imported.contentMd, /outside !\[Image\]\(__APP_ID__-upload:\/\//);
});
...
test("reference definitions work in repeated blockquotes and list containers", () => {
  ...
  assert.match(imported.contentMd, /^>>   \[deep\]: __APP_ID__-upload:\/\//m);
  assert.match(imported.contentMd, /^     \[plate\]: __APP_ID__-upload:\/\//m);
});
