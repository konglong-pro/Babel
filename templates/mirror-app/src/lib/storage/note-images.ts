...
export const NOTE_UPLOAD_PLACEHOLDER_PREFIX = "__APP_ID__-upload://";
...
function uploadTokensInMarkdown(contentMd: string): Set<string> {
  ...
  const pattern = /__APP_ID__-upload:\/\/([^\s)\]}'\"<>]+)/g;
...
function resolveNoteImageRoot(): string {
  const configuredUploadDirectory = process.env.__APP_ENV_PREFIX___UPLOAD_DIRECTORY;
...
const markdownBaseUrl = new URL("https://__APP_ID__.invalid/notes");
